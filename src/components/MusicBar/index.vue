  ...
  <div id="musicBarContainer">
    <div id="musicBar"
      ...
      <div class='music'>
        ...
        <audio id='audio'
               autoplay="autoplay"
               :src="audio_data.sound.source"></audio>
        <!-- 封面 -->
        <router-link class="music-cover"
                     :to="{ path: 'detail', query: { id: audio_data.sound.id } }">
        ...
        <div class="music-control">
          <!-- 播放列表 -->
          <div class="my-icon-menu control-icon"
               @click="$refs.popup.toggleVisible()"></div>
          <!-- 播放/暂停 -->
          <div class="control-icon control-icon-mid"
               :class="audio_play ? 'my-icon-pause' : 'my-icon-arrow'"
               @click="SET_AUDIO_PLAY(!audio_play)"></div>
          <!-- 下一首 -->
          <div class="my-icon-next control-icon"
               @click="handleNextAudio"></div>
        </div>
      </div>
      <!-- 进度条 -->
      <div class="progress_bar">
        <div class="progress_bar_inner"
             :style="`width:${audio_progress}`"></div>
      </div>
    </div>
    <!-- 播放列表 & 播放模式 -->
    <app-popup ref="popup"
...
export default {
  ...
  methods: {
    ...
    randomMode() {
      ...
      let isCurrentSame =
        this.playList[index].sound.id === this.audio_data.sound.id
      if (isCurrentSame) {
        this.listRepeatMode()
      } else {
    ...
    listRepeatMode() {
      ...
      let currentIndex = this.playList.findIndex(
        (n) => n.sound.id === this.audio_data.sound.id
      )
      if (currentIndex > -1) {
        let nextIndex
        ...
        let isCurrentSame =
          this.playList[nextIndex].sound.id === this.audio_data.sound.id
        if (isCurrentSame) {
          this.singleRepeatMode()
        } else {
