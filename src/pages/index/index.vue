<template>
  <div id='index'
       v-infinite-scroll="getListDataMore"
    ...
    <div class="recommend">
      ...
      <div class="playAll"
        ...
        <div class="playAll-label"
             type="default">一键播放</div>
      </div>
      <!-- 列表 -->
...
export default {
  ...
  methods: {
    ...mapMutations(['SET_AUDIO_DATA', 'SET_PLAY_MODE', 'SET_PLAY_LIST']),
    getBannerData() {
      getBanner().then((res) => {
    ...
    playAll() {
      ...
      if (
        this.audio.data &&
        this.listJson[0].sound.id === this.audio.data.sound.id
      ) {
        this.audio.ele.load()
        this.audio.ele.play()
      } else {
